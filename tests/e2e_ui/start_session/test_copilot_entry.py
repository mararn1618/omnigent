"""E2E: the seeded copilot agent and its model/effort config modal.

Covers the copilot chat flow in the landing composer (``NewChatLandingScreen``
in ``web/src/shell/NewChatDialog.tsx``) end to end: the server-seeded
``copilot`` agent lists among the built-in bundle agents, its gear modal
offers the host-resolved model catalog with per-model reasoning efforts
under the Agent Harness row, and the picked model + effort reach the create
call as ``model_override`` / ``reasoning_effort``.
...
def _copilot_agents_body() -> str:
    ...
    ``name: "copilot"`` is what lists the row among the built-in bundle
    agents (the picker's builtin set is name-keyed); ``harness: "copilot"``
    is what keys the gear modal's Model/Effort rows to the copilot catalog.
    Sole agent, so it auto-selects.
    """
    return json.dumps(
        {
            "data": [
...
def test_copilot_agent_model_and_effort_reach_the_create(
    ...
    """The copilot agent is pickable and its model/effort pick reaches the create.

    Asserts the three behaviors this feature adds to the new chat flow: the
    seeded copilot agent lists inline among the built-in agents, the gear
    modal renders the host-resolved catalog with per-model efforts (no
    effort row for Default/auto or an effort-less model, the full ladder
    including Max for a reasoning model), and the committed pick reaches
    ``POST /v1/sessions`` as ``model_override`` + ``reasoning_effort`` with
    no harness override (the agent already runs on copilot).
    """
...
async def _drive_copilot_agent(base_url: str, session_id: str) -> None:
    async with async_playwright() as pw:
        ...
        try:
            ...
            # The seeded copilot agent lists inline among the built-in bundle
            # agents: one click, no submenu drilling.
